...
<instructions>
  <workflow_execution>
    ...
    <stage id="2" name="ElevateCriticalRules" priority="HIGHEST" enforce="@position_sensitivity">
      ...
      <research_basis>Position sensitivity: early placement improves adherence (research: magnitude varies by task/model)</research_basis>
      <process>
    ...
    <stage id="3" name="FlattenNesting" enforce="@nesting_limit">
      <action>Reduce nesting from 6-7 to 3-4 levels (per @nesting_limit)</action>
      <prerequisites>Critical rules elevated (per Stage 2)</prerequisites>
      <research_basis>Excessive nesting reduces clarity (research: magnitude varies by task/model) - per @nesting_limit</research_basis>
      <process>
        1. Identify deeply nested sections (>4 levels) - violates @nesting_limit
        2. Convert nested elements→attributes where possible (per @nesting_limit strategy)
        3. Extract verbose sections→external refs (per modular design principle)
        4. Flatten decision trees using attributes (per @nesting_limit approach)
        5. Verify max depth ≤4 (per @nesting_limit compliance)
      </process>
    ...
    <stage id="6" name="ConsolidateRepetition" enforce="@single_source">
      <action>Implement single source of truth w/ @references (per @single_source)</action>
      <prerequisites>Instruction ratio optimized (per Stage 5, @instruction_ratio)</prerequisites>
      <research_basis>Eliminates ambiguity, improves consistency (per @single_source principle)</research_basis>
      <process>
        ...
        3. Replace repetitions w/ @rule_id or @section_id (per @single_source pattern)
        4. Verify refs work correctly (validate @single_source implementation)
        5. Test enforcement still applies (per critical_rules validation)
      </process>
      <reference_syntax>
        <definition>
          <critical_rules>
    ...
    <stage id="7" name="AddExplicitPriority" enforce="@execution_priority">
      <action>Create 3-tier priority system for conflict resolution (per @execution_priority)</action>
      <prerequisites>Repetition consolidated (per Stage 6, @single_source)</prerequisites>
      <research_basis>Resolves ambiguous cases, improves decision clarity - per @execution_priority principle</research_basis>
      <process>
        ...
        5. Document edge cases w/ examples for clarity
      </process>
      <template>
        <execution_priority>
    ...
    <stage id="8" name="StandardizeFormatting">
      <action>Ensure consistent attribute usage & XML structure (per quality_standards)</action>
      <prerequisites>Priority system added (per Stage 7, @execution_priority)</prerequisites>
      <process>
        1. Review all XML elements for consistency
        2. Convert metadata→attributes (id, name, when, required, etc.) - per quality_standards
    ...
    <stage id="9" name="EnhanceWorkflow">
      <action>Transform linear instructions→multi-stage executable workflow (per Tier 3: Enhancement)</action>
      <prerequisites>Formatting standardized (per Stage 8, quality_standards)</prerequisites>
      <routing_decision>
        <if condition="simple_prompt">Basic step-by-step w/ validation checkpoints (appropriate for simple complexity)</if>
        <if condition="moderate_prompt">Multi-step workflow w/ decision points (appropriate for moderate complexity)</if>
        <if condition="complex_prompt">Full stage-based workflow w/ routing intelligence (appropriate for complex - per Tier 3)</if>
      </routing_decision>
      <process>
    ...
    <stage id="10" name="ValidateOptimization">
      <action>Validate against all research patterns + calculate gains (per @critical_rules enforcement)</action>
      <prerequisites>All optimization stages complete (Stages 1-9, @execution_priority Tier 1-3)</prerequisites>
      <validation_checklist>
        <critical_position>✓ Critical rules <15% (per @position_sensitivity)</critical_position>
         ...
         <modular_design>✓ External refs for verbose sections (per modular_design principle)</modular_design>
         <token_efficiency>✓ 30-50% token reduction achieved (when applied)</token_efficiency>
         <semantic_preservation>✓ 100% meaning preserved (critical requirement)</semantic_preservation>
      </validation_checklist>
      <pattern_compliance>
