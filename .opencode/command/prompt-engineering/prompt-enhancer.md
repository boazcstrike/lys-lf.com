...
<instructions>
  <workflow_execution>
    ...
    <stage id="2" name="ElevateCriticalRules" priority="HIGHEST" enforce="@position_sensitivity">
      <action>Move critical rules to first 15% of prompt (per @position_sensitivity)</action>
      <prerequisites>Analysis complete, critical rules identified</prerequisites>
      <research_basis>Position sensitivity research: early placement improves adherence (per @position_sensitivity - effect varies by task/model)</research_basis>
      <process>
        1. Extract all critical/safety rules from prompt
        2. Create <critical_rules> block
        3. Position immediately after <role> (per @position_sensitivity: within first 15%)
        4. Assign unique IDs to each rule (per @single_source: define once)
        5. Replace later occurrences with @rule_id references (per @single_source: eliminate repetition)
        6. Verify position percentage <15% (per @position_sensitivity compliance)
      </process>
      <template>
        <critical_rules priority="absolute" enforcement="strict">
    ...
    <stage id="3" name="FlattenNesting" enforce="@nesting_limit">
      <action>Reduce nesting depth from 6-7 to 3-4 levels (per @nesting_limit)</action>
      <prerequisites>Critical rules elevated (per Stage 2, @position_sensitivity)</prerequisites>
      <research_basis>Excessive nesting reduces clarity (per @nesting_limit - magnitude varies by task/model)</research_basis>
      <process>
        1. Identify deeply nested sections (>4 levels) - violates @nesting_limit
        2. Convert nested elements to attributes where possible (per @nesting_limit strategy)
        3. Extract verbose sections to external references (per modular_design principle)
        4. Flatten decision trees using attributes (per @nesting_limit approach)
        5. Verify max depth ≤4 levels (per @nesting_limit compliance)
      </process>
    ...
    <stage id="4" name="OptimizeInstructionRatio" enforce="@instruction_ratio">
      <action>Reduce instruction ratio to 40-50% of total prompt (per @instruction_ratio)</action>
      <prerequisites>Nesting flattened (per Stage 3, @nesting_limit)</prerequisites>
      <research_basis>Optimal balance: 40-50% instructions, rest distributed across other components (per @instruction_ratio)</research_basis>
      <process>
        1. Calculate current instruction percentage (baseline for @instruction_ratio)
        2. If >60%, identify verbose sections to extract (violates @instruction_ratio)
        3. Create external reference files for:
           - Detailed specifications
           - Complex workflows
           - Extensive examples
           - Implementation details
        4. Replace with <references> section (per modular_design principle)
        5. Recalculate ratio, verify 40-50% (per @instruction_ratio compliance)
      </process>
    ...
    <stage id="5" name="ConsolidateRepetition" enforce="@single_source">
      <action>Implement single source of truth with @references (per @single_source)</action>
      <prerequisites>Instruction ratio optimized (per Stage 4, @instruction_ratio)</prerequisites>
      <research_basis>Eliminates ambiguity and improves consistency (per @single_source principle - effect varies by task/model)</research_basis>
      <process>
        1. Find all repeated rules/instructions (audit against @single_source requirement)
        2. Keep single definition in <critical_rules> or appropriate section (per @single_source)
        3. Replace repetitions with @rule_id or @section_id (per @single_source pattern)
        4. Verify references work correctly (validate @single_source implementation)
        5. Test that enforcement still applies (per critical_rules validation)
      </process>
      <reference_syntax>
        <definition>
          <critical_rules>
    ...
    <stage id="6" name="AddExplicitPriority" enforce="@execution_priority">
      <action>Create 3-tier priority system for conflict resolution (per @execution_priority)</action>
      <prerequisites>Repetition consolidated (per Stage 5, @single_source)</prerequisites>
      <research_basis>Resolves ambiguous cases and improves decision clarity (per @execution_priority principle - effect varies by task/model)</research_basis>
      <process>
        1. Identify potential conflicts in rules and workflow
        2. Create <execution_priority> section with 3-tier model
        3. Define 3 tiers: Safety/Critical → Core Workflow → Optimization (per @execution_priority)
        4. Add conflict_resolution rules (Tier 1 overrides Tier 2/3 per @critical_rules override)
        5. Document edge cases with examples for clarity
      </process>
      <template>
        <execution_priority>
    ...
    <stage id="7" name="StandardizeFormatting">
      <action>Ensure consistent attribute usage and XML structure (per quality_standards)</action>
      <prerequisites>Priority system added (per Stage 6, @execution_priority)</prerequisites>
      <process>
        1. Review all XML elements for consistency
        2. Convert metadata to attributes (id, name, when, required, etc.) - per quality_standards
        3. Keep content in nested elements (per quality_standards.pattern_compliance)
        4. Standardize attribute order: id, name, type, when, required, enforce, other
        5. Verify XML validity and consistency
      </process>
      <standards>
    ...
    <stage id="8" name="EnhanceWorkflow">
      <action>Transform linear instructions into multi-stage executable workflow (per Tier 3: Enhancement)</action>
      <prerequisites>Formatting standardized (per Stage 7, quality_standards)</prerequisites>
      <routing_decision>
        <if condition="simple_prompt">
          <apply>Basic step-by-step with validation checkpoints (appropriate for simple complexity)</apply>
        </if>
        <if condition="moderate_prompt">
          <apply>Multi-step workflow with decision points (appropriate for moderate complexity)</apply>
        </if>
        <if condition="complex_prompt">
          <apply>Full stage-based workflow with routing intelligence (appropriate for complex - per Tier 3)</apply>
        </if>
      </routing_decision>
      <process>
        <simple_enhancement>
    ...
    <stage id="9" name="ValidateOptimization">
      <action>Validate against all research patterns and calculate gains (per @critical_rules enforcement)</action>
      <prerequisites>All optimization stages complete (Stages 1-8, @execution_priority Tier 1-3)</prerequisites>
      <validation_checklist>
        <critical_position>✓ Critical rules in first 15% (per @position_sensitivity)</critical_position>
        <nesting_depth>✓ Max depth ≤4 levels (per @nesting_limit)</nesting_depth>
        <instruction_ratio>✓ Instructions 40-50% (per @instruction_ratio)</instruction_ratio>
        <single_source>✓ No rule repeated >2x (per @single_source)</single_source>
        <explicit_priority>✓ 3-tier priority system exists (per @execution_priority)</explicit_priority>
        <consistent_format>✓ Attributes used consistently (per quality_standards)</consistent_format>
        <modular_design>✓ External references for verbose sections (per modular_design principle)</modular_design>
      </validation_checklist>
